	...
	DWORD WINAPI IPCThread(LPVOID lpParam) {
		...
		while (true) {
			if (pSharedData->execute) { // 请求检测
				...
				{
				case static_cast<int>(Command::COMMAND_MOVE_ICON):
					this->ProcessMoveRequest(pSharedData);
					logMessage.log(L"请求处理完成: 移动图标（坐标）");
					break;
				case static_cast<int>(Command::COMMAND_MOVE_ICON_BY_RATE):
					this->ProcessMoveRequest(pSharedData, true);
					logMessage.log(L"请求处理完成: 移动图标（比率）");
					break;
				case static_cast<int>(Command::COMMAND_REFRESH_DESKTOP):
					this->RefreshDesktop();
					logMessage.log(L"请求处理完成: 刷新桌面");
					break;
				case static_cast<int>(Command::COMMAND_SHOW_DESKTOP):
					this->ShowDesktop();
					logMessage.log(L"请求处理完成: 显示桌面");
					break;
				case static_cast<int>(Command::COMMAND_IS_OK):
					logMessage.log(L"状态良好");
					break;
				case static_cast<int>(Command::COMMAND_GET_ICON):
					this->ProcessGetAllIconsRequest(pSharedData);
					logMessage.log(L"请求处理完成: 获取桌面上所有图标");
					break;
				case static_cast<int>(Command::COMMAND_GET_ICON_NUMBER):
					this->ProcessGetIconNumberRequest(pSharedData);
...
private:
	...
	bool ProcessGetAllIconsRequest(SharedData* pSharedData)
	{
		...
		pSharedData->size = GetAllIcons(hListView, pSharedData->iconPositionMove, pSharedData->u_j, pSharedData->size);
		return true;
	}

	bool ProcessGetIconNumberRequest(SharedData* pSharedData)
	{
		HWND hListView = GetLocalHListView();
		if (hListView == nullptr)
			return false;
		pSharedData->size = this->GetIconsNumber(hListView);
		return true;
	}
	...
	wstring MoveDesktopIcon(HWND hListView, int index, int x, int y, bool is_rate = false) {
		...
		if (is_rate)
		{
			...
			x = static_cast<int>(cx * (static_cast<double>(x) / 100000));
			y = static_cast<int>(cy * (static_cast<double>(y) / 100000));
		}
	...
	size_t GetAllIcons(HWND hListView, IconPositionMove* IconPositionMove, size_t j, size_t maxSizeOnce = MAX_ICON_COUNT)
	{
	...
	size_t GetIconsNumber(HWND hListView)
	{
		if (!hListView) return 0;
