...
class SaveLayoutCommand : public Command {
	...
	std::wstring sortMode;
	bool outputToConsole;
public:
	SaveLayoutCommand(const std::wstring& path, const std::wstring& sort, bool output)
...
class SortLayoutCommand : public Command {
	...
	std::wstring sortMode;
public:
	SortLayoutCommand(const std::wstring& path, const std::wstring& sort)
	...
	bool execute(LogMessage& logger, Mover&, DataManager& dm) override {
		...
		// 保存结果
		std::wstring sortedPath = filePath + L"_sorted.bin";
...
class SaveFullLayoutCommand : public Command {
	...
	bool outputToConsole;
public:
	SaveFullLayoutCommand(const std::wstring& path, const std::wstring& sort, bool output)
		: filePath(path), sortMode(sort), outputToConsole(output) {
	}
...
class MoveIconsCommand : public Command {
	std::wstring filePath;
public:
	explicit MoveIconsCommand(const std::wstring& path) : filePath(path) {}
	...
	bool execute(LogMessage& logger, Mover& mover, DataManager& dm) override {
		...
		logger.log(L"成功读取布局文件: " + filePath + L"，包含 " +
			std::to_wstring(ratioPoints.size()) + L" 个点");
...
public:
	struct Options {
		std::wstring operationMode;
		std::wstring filePath = L".\\rikka.bin";
		std::wstring sortMode;
		std::wstring injectMode = L"auto";
	...
	unique_ptr<Command> createCommand() {
		...
		if (argc == 1)										return std::unique_ptr<Command>(new ClearDesktopCommand());
		if (options.injectMode == L"unset")					return std::unique_ptr<Command>(new UnsetCommand());
		if (options.operationMode == L"save")				return std::unique_ptr<Command>(new SaveLayoutCommand(options.filePath, options.sortMode, options.outputToConsole));
		if (options.operationMode == L"save-full")			return std::unique_ptr<Command>(new SaveFullLayoutCommand(options.filePath, options.sortMode, options.outputToConsole));
		if (options.operationMode == L"move")				return std::unique_ptr<Command>(new MoveIconsCommand(options.filePath));
		if (options.operationMode == L"sort")				return std::unique_ptr<Command>(new SortLayoutCommand(options.filePath, options.sortMode));
		if (options.operationMode == L"clear")				return std::unique_ptr<Command>(new ClearDesktopCommand());
		if (options.operationMode == L"666")				return std::unique_ptr<Command>(new Special666Command());
		if (options.operationMode == L"windows")			return std::unique_ptr<Command>(new SpecialWindowsCommand());
		if (options.operationMode == L"clearlog")			return std::unique_ptr<Command>(new ClearLogFileCommand());
		if (options.operationMode == L"restart-explorer")	return std::unique_ptr<Command>(new RestartExplorerCommand());
		return nullptr;
	}
...
private:
	...
	Options options;
	std::map<std::wstring, int> optionCounters;
	...
	void parseArguments(int argc, wchar_t* argv[]) {
		for (int i = 1; i < argc; ++i) {
			...
			std::wstring key = (pos != std::wstring::npos) ? arg.substr(0, pos) : arg;
			std::wstring value = (pos != std::wstring::npos) ? arg.substr(pos + 1) : L"";
	...
	void validateOptions() {
		...
		for (const auto& pair : optionCounters) {
			...
			if (count > 1 && key != L"--output") {
				throw std::runtime_error("重复选项: " + toNarrow(key));
			}
		}

		// 验证操作模式
		static const std::vector<std::wstring> validModes = {
			L"save", L"save-full", L"move", L"sort", L"clear", L"666", L"windows", L"clearlog", L"restart-explorer"
		};

		if (!options.operationMode.empty() &&
			std::find(validModes.begin(), validModes.end(), options.operationMode) == validModes.end()) {
			throw std::runtime_error("无效的操作模式");
		}

		// 验证注入模式
		static const std::vector<std::wstring> validInjectModes = {
			L"true", L"false", L"auto", L"unset"
		};

		if (!options.injectMode.empty() &&
			std::find(validInjectModes.begin(), validInjectModes.end(), options.injectMode) == validInjectModes.end()) {
			throw std::runtime_error("无效的注入模式");
		}
	}
...
public:
	...
	int run() {
		try {
			...
			// 处理注入模式
			if (parser.getOptions().injectMode == L"unset") {
				if (mover.UnInjectDLL()) {
					logger.log(L"DLL卸载成功");
					return EXIT_SUCCESS;
				}
				logger.error(L"DLL卸载失败");
				return EXIT_FAILURE;
			}
			...
			logger.error(L"命令行解析错误: " + std::wstring(e.what(), e.what() + strlen(e.what())));
			return EXIT_FAILURE;
		}
	}
